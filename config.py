...
_source_type = _env('SOURCE_TYPE', _yaml['source']['type'])
# OneDrive mode doesn't need a local PHOTOS_DIR; local mode does.
if not _photos_raw and _source_type != 'onedrive':
    raise EnvironmentError(
        "photos_dir is not configured.\n"
        "Set it in config/config.yaml  →  paths.photos_dir: D:\\MyPhotos\n"
        "  or in .env                  →  PHOTOS_DIR=D:\\MyPhotos"
    )
PHOTOS_DIR = Path(_photos_raw).resolve() if _photos_raw else Path('/nonexistent')
...
ONEDRIVE_INDEX_PATH   = _env('ONEDRIVE_INDEX_PATH',   _yaml['source']['onedrive']['index_path'])
